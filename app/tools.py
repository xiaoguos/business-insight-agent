...
def query_readonly(path, sql, params=(), timeout=.5):
    ...
    tree = expressions[0]
    forbidden = (exp.Insert, exp.Delete, exp.Update, exp.Create, exp.Drop, exp.Command, exp.Union)
    if any(isinstance(node, forbidden) for node in tree.walk()):
        raise UnsafeQuery("查询包含禁止操作")
    tables = list(tree.find_all(exp.Table))
    if not tables or any(table.name != "orders" or table.db or table.catalog for table in tables):
...
def summarize(path, dimension="channel", channel=None, product=None):
    ...
    return {"sql": sql, "params": params, "rows": query_readonly(path, sql, params), "source": "orders", "window": {"baseline": "2026-09-01—2026-09-07", "current": "2026-09-08—2026-09-14"}}


def analyze(result):
    rows = result["rows"]
    by_period = {p: [row for row in rows if row["period"] == p] for p in ["baseline", "current"]}
    ...
    for period, records in by_period.items():
        ...
        totals[period] = {"orders": orders, "refunds": refunds, "rate": refunds / orders}
    segments = []
    for row in by_period["current"]:
        before = next((r for r in by_period["baseline"] if r["segment"] == row["segment"]), None)
        ...
        segments.append({"segment": row["segment"], "baseline_rate": baseline, "current_rate": current, "delta_pp": (current - baseline) * 100, "excess_refunds": excess, "orders": row["orders"]})
    return {"totals": totals, "delta_pp": (totals["current"]["rate"] - totals["baseline"]["rate"]) * 100, "segments": sorted(segments, key=lambda row: row["excess_refunds"], reverse=True), "method": "分组退款率与基准率下的超额退款量；描述性分析，不证明因果"}
...
def retrieve_rules(query):
    ...
    scored = [(len(terms & {r["text"][i:i + 2] for i in range(len(r["text"]) - 1)}), r) for r in rules]
    return [r for score, r in sorted(scored, key=lambda pair: -pair[0]) if score > 0][:3]


def compose_report(question, plan, result, analysis, rules):
    totals = analysis["totals"]
    lead = analysis["segments"][0] if analysis["segments"] else None
    lines = ["# 退款率异常分析", "", "数据来源：固定种子生成的模拟订单，不代表真实企业经营。", f"问题：{question}", f"口径：退款订单数 / 已创建订单数；按 {plan['dimension']} 拆解。", f"基准周期：{result['window']['baseline']}；当前周期：{result['window']['current']}。", "", f"- 基准期：{totals['baseline']['refunds']}/{totals['baseline']['orders']}，退款率 {totals['baseline']['rate']:.2%}。", f"- 当前期：{totals['current']['refunds']}/{totals['current']['orders']}，退款率 {totals['current']['rate']:.2%}。", f"- 变化：{analysis['delta_pp']:+.2f} 个百分点。"]
    if lead:
        lines += [f"- 超额退款量最高的分组：{lead['segment']}，退款率变化 {lead['delta_pp']:+.2f} 个百分点。"]
    lines += ["", "## 证据与限制", "上述数值由 SQL 结果和 Python 计算产生。当前证据只能定位异常分组，不能证明退款的业务原因。"]
    lines += [f"- [{rule['id']}] {rule['text']}" for rule in rules] or ["- 业务规则检索暂不可用，报告仅包含数据事实。"]
    lines += ["", "## 下一步核查", "核对异常分组的退款原因码、活动记录和订单明细，再决定是否调整投放。", "", "```sql", result["sql"], "```", "参数：" + json.dumps(result["params"], ensure_ascii=False)]
    return "\n".join(lines)
