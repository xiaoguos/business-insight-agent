...
def plan_question(question, provider="rules"):
    ...
    if dates or any(
        ...
        return Plan(
            supported=False,
            clarification="当前仅支持页面列出的两个固定 7 天退款率窗口，暂不支持指定日期或其他指标；不会用整周数据替代单日查询。",
        )
    if provider == "rules":
        if "退款" not in question:
            return Plan(
                supported=False,
                clarification="当前支持 2026-09-01 至 09-14 模拟数据的退款率分析，请明确按渠道或商品拆解。",
            )
        dimension = (
            "product"
            if any(word in question for word in ["商品", "产品", "版本"])
            else "channel"
        )
        channel = next(
            (
                value
                for value in ["自然流量", "广告投放", "合作渠道"]
                if value in question
            ),
            None,
        )
        product = next(
            (value for value in ["标准版", "专业版"] if value in question), None
        )
