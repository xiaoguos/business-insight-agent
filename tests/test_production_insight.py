...
def setup_task(environment):
    app=create_app(environment,planner=lambda q,c:{"dimension":"channel","channel":None,"product":None,"metric":"refund_rate"})
    ...
    analyst=headers(client,"analyst@example.test");reviewer=headers(client,"reviewer@example.test")
    response=client.post("/api/datasets",headers=admin,files={"file":("orders.csv",CSV)})
    assert response.status_code==201,response.text
    payload={"question":"分析退款率按渠道","dataset_id":response.json()["id"],"baseline_start":"2026-08-01","baseline_end":"2026-08-31","current_start":"2026-09-01","current_end":"2026-09-30","request_key":"test-request-key"}
...
def test_real_import_graph_review_and_notification(environment):
    ...
    assert client.post("/api/tasks",headers=analyst,json={**payload,"question":"different"}).status_code==409
    ...
    review={"approved":True,"report_hash":task["report_hash"],"note":"已核对源数据口径"}
    assert client.post("/api/tasks/"+task["id"]+"/approval",headers=analyst,json=review).status_code==403
    assert client.post("/api/tasks/"+task["id"]+"/approval",headers=reviewer,json={**review,"report_hash":"0"*64}).status_code==409
    response=client.post("/api/tasks/"+task["id"]+"/approval",headers=reviewer,json=review)
    assert response.status_code==200,response.text
    assert client.post("/api/tasks/"+task["id"]+"/approval",headers=reviewer,json=review).status_code==409
    app.state.handle_job(environment.claim())
    ...
    with environment.Session() as db:assert len(db.scalars(select(Notification)).all())==1
...
def test_cross_tenant_and_invalid_import(environment):
    ...
    assert client.post("/api/datasets",headers=analyst,files={"file":("orders.csv",CSV)}).status_code==403
    bad=CSV+CSV.splitlines()[1]+b"\n"
    assert client.post("/api/datasets",headers=admin,files={"file":("orders.csv",bad)}).status_code==422
...
def test_provider_failure_has_no_fabricated_report_and_retry(environment):
    ...
    app.state.insight.planner=fail
    job=environment.claim()
    try:app.state.handle_job(job)
    except ValueError:environment.fail(job,"model unavailable")
    ...
    assert client.post("/api/tasks/"+task["id"]+"/retry",headers=analyst).status_code==409
    app.state.insight.planner=lambda q,c:{"dimension":"product","metric":"refund_rate"}
    app.state.handle_job(environment.claim())
    assert client.get("/api/tasks/"+task["id"],headers=analyst).json()["state"]=="awaiting_approval"
...
def test_four_eyes_even_for_admin(environment):
    ...
    assert client.post("/api/tasks/"+task["id"]+"/approval",headers=admin,json={"approved":True,"report_hash":task["report_hash"],"note":"self review"}).status_code==403
